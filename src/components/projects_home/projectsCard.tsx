...
function renderPriority(level: string) {
  ...
  if (level === "H") {
    return <button className={`${style["high"]}`}>HIGH PRIORITY</button>;
  } else if (level === "M") {
    return (
      <button className={`${style["medium"]} medium`}>MEDIUM PRIORITY</button>
    );
  } else {
    return <button className={`${style["low"]} low`}>LOW PRIORITY</button>;
  }
}
...
function ProjectCard(props: any) {
  ...
  function renderIconOrRenderFirstLetterInName(icon_url: any) {
    ...
    if (icon_url != null) {
      ...
      return (
        <span className={style["icon-no-show"]}>{props.title.slice(0, 1)}</span>
      );
    }
  }
    ...
    if (contributor.profile_image != null) {
      return (
        <div className={style["tooltip"]}>
          <div
            style={contributorBackGround}
            className={style["contributor-avatar"]}
          >
        ...
        <div className={style["project-card-body"]}>
          ...
          <div className={style["progress-group"]}>
            <div className={style["tasks-completed-back"]}>
              <div
                className={style["tasks-completed"]}
                style={widthStyle}
              ></div>
            </div>
          </div>
          ...
          <div className={style["project-contributors-avatars"]}>
            {props.contributors.map((contributer: any) => {
              return renderAvatarOrFirstLetterInName(contributer);
            })}
            {props.contributors.length > 7 ? (
              <div className={style["more-avatar"]}>
                +{props.contributors.length - 7}
              </div>
            ) : (
              ""
            )}
            {props.contributors.length < 1 ? "No contributors" : ""}
          </div>

          <div className={style["project-due-date"]}>
            <div className={style["due-date"]}>CREATED: {props.timesince}</div>
          </div>
        </div>
      </Link>
    </div>
